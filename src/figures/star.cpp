...
Star::Star(int nsides, qreal dd, int ss) : RadialFigure(nsides)
{
    d = clamp_d(dd);
    s = clamp_s(ss);
...
Star::Star(const Figure & fig,  int nsides, qreal dd, int ss) : RadialFigure(fig, nsides)
{
    d = clamp_d(dd);
    s = clamp_s(ss);
    setFigType(FIG_TYPE_STAR);
...
MapPtr Star::buildUnit()
{
    ...
    if (debug) debugMap = make_shared<Map>("star debug map");
