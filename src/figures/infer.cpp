...
QList<intersection_info*> Infer::buildIntersectionInfos( int side, QPointF sideHalf, bool isLeftHalf, qreal requiredRotation, QPolygonF points, QPolygonF midPoints )
{
    ...
    // Sort edge-to-edge intersection by their total length.
    QMultiMap<qreal,intersection_info*> sortMap;
    for (int i= 0; i < infos.size(); i++)
    {
        sortMap.insert(infos[i]->dist2,infos[i]);   // insert is sorted on key
    }
...
{
    MapPtr ret = make_shared<Map>("infer map");    // the inferred map
    ...
    if (cons.size() == 0)
    {
        ...
        // DAC TODO - an approach would be to use the mid-points of the feature
        return ret;
    }
