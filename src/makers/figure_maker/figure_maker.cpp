...
FigureMaker * FigureMaker::mpThis = nullptr;

FigureMaker * FigureMaker::getInstance()
{
    if (mpThis == nullptr)
    {
        mpThis = new FigureMaker;
    }
    return mpThis;
}
...
void FigureMaker::init(TiledPatternMaker * maker, page_figure_maker * menu)
{
    this->maker = maker;
    this->menu  = menu;
...
void FigureMaker::slot_figureChanged()
{
    ...
    workspace->setSelectedDesignElement(wsdata,dep);

    emit sig_viewWS();
}

// called from page_figure_editor
void FigureMaker::setFromTiling(eWsData wsdata)
{
    masterEdit->reset();

    PrototypePtr pp = getPrototype(wsdata); //  creates a new prototype if none
    if (!pp)
    {
        TilingPtr tp = workspace->getTiling(wsdata);
        if (tp)
        {
            pp = make_shared<Prototype>(tp);
            //pp = createPrototypeFromLauncherButtons();
            workspace->setPrototype(wsdata,pp);
        }
    }

    FeatureBtnPtr btn = launcher->launchFromPrototype(pp);
    launcher->setCurrentButton(btn);
}

// called from page_figure_editor
void FigureMaker::setFromStyledDesign(eWsData wsdata)
{
    masterEdit->reset();

    StyledDesign & sd = workspace->getStyledDesign(wsdata);
    StylePtr sp       = sd.getFirstStyle();
    FeatureBtnPtr btn = launcher->launchFromStyle(sp);
    launcher->setCurrentButton(btn);

#if 0
    // remove 25APR2020
    eWsData wsdata = (config->figureViewer == FV_STYLE) ? WS_LOADED : WS_TILING;
    workspace->setPrototype(wsdata,designPrototype);
#endif
}
...
bool FigureMaker::duplicateActiveFeature(eWsData wsdata)
{
...
StylePtr  FigureMaker::createDefaultStyleFromPrototype()
{
    ...
    PrototypePtr pp = getPrototype(input_wsdata);
...
PrototypePtr FigureMaker::getPrototype(eWsData wsdata)
{
    PrototypePtr pp = workspace->getPrototype(wsdata);
    return pp;
}
...
{
    eWsData input_wsdata  = (config->figureViewer == FV_STYLE) ? WS_LOADED : WS_TILING;
    PrototypePtr proto = getPrototype(input_wsdata);
    try
    {
