...
public:
    static FigureMaker * getInstance();

    void init(TiledPatternMaker * maker, page_figure_maker * menu);
    void unload();

    void setFromTiling(eWsData wsdata);
    void setFromStyledDesign(eWsData wsdata);

    void        setActiveFeature(FeatureBtnPtr fb);
    FeaturePtr  getActiveFeature();
    bool        duplicateActiveFeature(eWsData wsdata);

    PrototypePtr getPrototype(eWsData wsdata);
...
protected:
     ...
     eWsData      input_wsdata;
     eWsData      output_wsdata;

private:
    FigureMaker();

    static FigureMaker * mpThis;
