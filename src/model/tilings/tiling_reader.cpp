...
TilingPtr TilingReader::readTilingXML(VersionedFile & xfile, MRBasePtr base)
{
    ...
    if (result == false)
    {
        qWarning() << "Badly constructed Tiling XML file" << xfile.getPathedName();
        return nullptr;
    }
...
TilingPtr TilingReader::readTilingXML(xml_node & tiling_node, MRBasePtr base)
{
    ...
    if (node)
    {
        // takes file name from XML for writing into the tiling
        vname.set(node.child_value());
    }
    else
    {
    ...
    for (xml_node tile_node = tiling_node.child(tileName.c_str()); tile_node; tile_node = tile_node.next_sibling(tileName.c_str()))
    {
        ...
        if (strtype == "girih")
        {
            ...
            if (pfp->loadFromGirihShape(vname))
            {
                tile = pfp->getTile();
            }
        }
