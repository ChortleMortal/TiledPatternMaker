...
void  page_modelSettings::onRefresh()
{
    ...
    if (tiling)
    {
        qDebug() << "Tiling" << tiling->getName().get();
    ...
    Layer * layer = Sys::view->getActiveLayer(viewType);
    if (layer)
    {
        l_xform->setText( "canvas " + Transform::info(layer->getCanvasTransform()));
        l_canvas->setText("model  " + Transform::info(layer->getModelTransform()));
        l_layer->setText( "layer  " + Transform::info(layer->getLayerTransform()));
    }
    else
    {
        l_canvas->clear();
