...
DCEL::DCEL(MapPtr map) : Map("DCEL")
{
    ...
    edges       = map->edges;
    qDebug().noquote() << "DECL: importing" << summary();
...
void DCEL::buildDCEL(Map * map)
{
    ...
    qDebug() << "DCEL: filling half-edge table";
    fill_half_edge_table(map);

    qDebug() << "DCEL: filling faces-table";
    fill_half_edge_faces();

    qDebug() << "DCEL: filling face-table inner componenents";
...
void DCEL::fill_half_edge_table(Map * map)
{
    ...
    for (const auto & edge : std::as_const(edges))
    {
        if (!edge->dvisited)
        {
            ...
            EdgePtr next    = next_half_edge(map,current);
            while (next != edge)
            {
                if (next)
                {
                    ...
                    current->next   = next;
                    current         = next;
                    next            = next_half_edge(map,current);
                }
                else
                {
...
EdgePtr DCEL::next_half_edge(Map * map, const EdgePtr & edge)
{
    qreal max_angle = 0.0;
    ...
    if (next)
    {
        return findEdge(map, v2 , next,true);
    }
    else
    {
        EdgePtr dp;
