...
CompareBMPStepper::CompareBMPStepper(ImageEngine * parent) : SteppingEngine(parent)
{
}

bool CompareBMPStepper::begin()
{
    if (isStarted())
    {
        end();
...
bool CompareBMPStepper::tick()
{
    return false;
}

bool CompareBMPStepper::next()
{
...
void CompareBMPStepper::resync(VersionedName name)
{
    if (!isStarted())
        return;
...
void CompareBMPStepper::setWorklist(VersionList &vlist)
{
    if (isStarted())
    {
